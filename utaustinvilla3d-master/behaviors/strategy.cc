...
SkillType NaoBehavior::selectSkill() {
    ...
    VecPosition Target[6] = {
        ...
        VecPosition(-15, (Posball.getY()) / 10 * 1.05, 0),
        Posball + VecPosition(9, 4, 0),
        Posball + VecPosition(9, -4, 0),
        VecPosition((Posball.getX() - 15.0) / 2. + 5 * sin(atan2(Posball.getY(), Posball.getX() - 15)), (Posball.getY() - 15.0) / 2.0 + 5 * cos(atan2(Posball.getY(), Posball.getX() - 15)), 0),
        VecPosition((Posball.getX() - 15.0) / 2. - 5 * sin(atan2(Posball.getY(), Posball.getX() - 15)), (Posball.getY() - 15.0) / 2.0 - 5 * cos(atan2(Posball.getY(), Posball.getX() - 15)), 0)};
        ...
        if (worldModel->getUNum() == WO_TEAMMATE1 + BotForTarget[i]) // 遍历到了我
            if (i == 0)
            {
                ...
                { // 若已到位就执⾏踢球
                    if (Posball.getX() < 9)
                    {
                        if (me.getDistanceTo(Target[1]) < me.getDistanceTo(Target[2]))
